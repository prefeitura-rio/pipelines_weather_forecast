...
from beartype.typing import Dict, Literal, Optional, Union
...
class ResnetBlock(Module):
    def __init__(self, dim, dim_out=None, *, cond_dim=None):
        ...
        if exists(cond_dim):
            self.mlp = Sequential(nn.ReLU(), nn.Linear(cond_dim, dim_out * 2))
...
class Attention(Module):
    def __init__(
        self, dim, cond_dim=None, heads=32, dim_head=32, dropout=0.0, window_size=8, num_registers=1
    ):
        super().__init__()
...
Predictions = namedtuple("Predictions", ["surface", "hrrr", "precipitation"])

LossBreakdown = namedtuple("LossBreakdown", ["surface", "hrrr", "precipitation"])
...
class MetNet3(Module):
    ...
    def __init__(
        self,
        *,
        dim=512,  # dim of the final
        num_lead_times=722,  # The 24 hours ahead spaced by 2 minutes.
        lead_time_embed_dim=32,  # The embeded of the times.
        input_spatial_size=624,  # Initial H and W dimention
        attn_depth=12,  #
        attn_dim_head=64,  #
        attn_heads=32,  #
        attn_dropout=0.1,  #
        ...
        hrrr_norm_strategy: Union[
            Literal["none"], Literal["precalculated"], Literal["sync_batchnorm"]
        ] = "none",
        hrrr_channels=617,
        ...
        self.resnet_blocks_down_4km = ResnetBlocks(
            dim=dim, dim_in=dim_in_4km, cond_dim=lead_time_embed_dim, depth=resnet_block_depth
        )
        ...
        self.resnet_blocks_down_8km = ResnetBlocks(
            dim=dim, dim_in=dim_in_8km, cond_dim=lead_time_embed_dim, depth=resnet_block_depth
        )

        self.downsample_to_16km = Downsample2x()

        self.vit = MaxViT(
            dim=dim,  # 512, porbably the channels
            depth=attn_depth,  # 12
        ...
        self.resnet_blocks_up_8km = ResnetBlocks(
            dim=dim, dim_in=dim + dim_in_8km, cond_dim=lead_time_embed_dim, depth=resnet_block_depth
        )

        self.linear_attn_8km = XCAttention(dim=dim, cond_dim=lead_time_embed_dim)
        ...
        self.resnet_blocks_up_4km = ResnetBlocks(
            dim=dim, dim_in=dim + dim_in_4km, cond_dim=lead_time_embed_dim, depth=resnet_block_depth
        )

        self.upsample_4x_to_1km = nn.ConvTranspose2d(dim, dim, kernel_size=4, stride=4)

        self.resnet_blocks_up_1km = ResnetBlocks(
            dim=dim, depth=resnet_block_depth, cond_dim=lead_time_embed_dim
        )
        ...
        self.to_precipitation_bins = Sequential(
            ChanLayerNorm(dim), nn.Conv2d(dim, sum(self.precipitation_bin_dims), 1)
        )
        ...
        self.to_surface_bins = Sequential(
            ChanLayerNorm(dim), nn.Conv2d(dim, sum(self.surface_bin_dims), 1)
        )
        ...
        if hrrr_norm_strategy == "none":
            ...
            assert exists(
                hrrr_norm_statistics
            ), "hrrr_norm_statistics must be passed in, if normalizing input hrrr as well as target with precalculated dataset mean and variance"
            assert hrrr_norm_statistics.shape == (
                2,
                hrrr_channels,
            ), f"normalization statistics must be of shape (2, {hrrr_channels}), containing mean and variance of each target calculated from the dataset"
    ...
    def save(self, path, overwrite=True):
        ...
        pkg = dict(model_state_dict=self.state_dict(), config=self._configs)
    ...
    def forward(
        ...
        *,
        ...
        loss_breakdown = LossBreakdown(
            surface_loss_breakdown, hrrr_loss, precipitation_loss_breakdown
        )
